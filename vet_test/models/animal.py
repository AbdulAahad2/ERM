...
class VetAnimal(models.Model):
    ...
    _sql_constraints = [
        ('microchip_unique', 'unique(microchip_no)', 'Animal ID must be unique!')
    ]
    ...
    owner_id = fields.Many2one('vet.animal.owner', string="Owner", tracking=True, index=True)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            ...
            # HANDLE PARTNER_ID → CREATE OWNER
            ...
            # VALIDATE MOBILE - Only validate if owner exists and has valid partner
            owner_id = vals_copy.get('owner_id')
            if owner_id:
                owner = self.env['vet.animal.owner'].browse(owner_id)
                if owner.exists() and owner.partner_id:
                    partner = owner.partner_id
                    # Only validate if this is a regular individual vet owner (not company, not user)
                    if not partner.is_company and not partner.user_ids:
                        mobile = partner.mobile
                        if not mobile:
                            raise ValidationError(_("Contact number must be set for the animal owner. Please update the owner's contact information."))
                        cleaned_mobile = re.sub(r'\D', '', mobile)
                        if not re.fullmatch(r"\d{11}", cleaned_mobile):
                            raise ValidationError(_("Mobile number must be exactly 11 digits for the animal owner."))
                        
                        self.env.cr.execute("""
                            SELECT id FROM vet_animal_owner 
                            WHERE contact_number = %s AND id != %s 
                            LIMIT 1
                        """, (cleaned_mobile, owner.id))
            ...
            if not microchip_no or microchip_no == "New":
                ...
                self.env.cr.execute("""
                    SELECT MAX(CAST(SUBSTRING(microchip_no FROM 5) AS INTEGER))
                    FROM vet_animal 
                    WHERE microchip_no LIKE 'PET-%' 
                        AND microchip_no ~ 'PET-[0-9]+'
                """)
                result = self.env.cr.fetchone()
                max_used = result[0] if result and result[0] else 0
    ...
    def init(self):
        ...
        self.env.cr.execute("""
            CREATE INDEX IF NOT EXISTS idx_vet_animal_owner 
            ON vet_animal(owner_id);
        """)
        
        # Index for name searches
        self.env.cr.execute("""
            CREATE INDEX IF NOT EXISTS idx_vet_animal_name 
            ON vet_animal(name);
        """)
        
        # Index for active animals
        self.env.cr.execute("""
            CREATE INDEX IF NOT EXISTS idx_vet_animal_active 
            ON vet_animal(active) WHERE active = true;
        """)
